...
type
  ...
  HANGLA = array [0..(HANMAX - 1)] of HANGLE;               // Hangleírók táblázata
...
function SLLDLL_LEDNyil(rgbert: HABASZ; nilmeg: Dword; amital: Word): Dword; stdcall; Assembler; Forward;
...
//
// Van olyan, amit megváltoztatna, megnézem a típusazonosságot
//
    mov   ax,amirev                     // Erre változtatna
...
asm
    ...
    call  ervazo                        // Azonosító lekérdezés
    jz    @renaza                       // Rendben van az azonosító
    mov   ax,amital                     // Ezt küldte
...
//                                                                                  //
//////////////////////////////////////////////////////////////////////////////////////
function SLLDLL_LEDNyil(rgbert: HABASZ; nilmeg: Dword; amital: Word): Dword; stdcall; Assembler;
asm
    mov   eax,ERROR_DLL_INIT_FAILED     // Hibakód
    mov   edx,[pwndfu]                  // WNDProc értéke
    or    edx,edx                       // Volt már?
...
//
    ...
    jz    @renaza                       // Rendben van az azonosító
//
// Nem az volt, LED lámpa típus vizsgálat
//
    mov   ax,amital                     // Ezt küldte
    mov   ecx,SLLELO                    // A kívánt elõtag
    call  ervazo                        // Azonosító lekérdezés
  ...
  @renaza:
    ...
    mov   eax,dword ptr rgbert          // Az RGB paraméter
    mov   dword ptr [edx + USB_buf.ledszb],eax// Átadom az RGB értékeket
    mov   ecx,nilmeg                    // Az irány
    or    ecx,ecx                       // Nulla, azaz balra?
    jz    @marafa                       // Igen, így jó is
    cmp   ecx,1                         // Jobbra?
    jz    @marafa                       // Igen, így jó is
    mov   eax,ERROR_INVALID_DATA        // Hibakód
    cmp   ecx,2                         // Lámpa legyen?
    jnz   @hibret                       // Nem jó az érték
  @marafa:
    mov   [edx + USB_buf.lednir],cl     // Átadom az irány értékeket
    push  edx                           // UsbHidWrite 2. paraméter, a puffer címe
    push  0                             // UsbHidWrite 1. paraméter, az eszköz sorszáma
...
//                ERROR_BAD_LENGTH            A hanghossz nagyobb mint 16           //
//                egyéb értékek               Windows mûveleti hibakódok            //
//                                                                                  //
//////////////////////////////////////////////////////////////////////////////////////
...
asm
    ...
    mov   eax,parrom                    // AX <- ezt kell megváltoztatni, a magas részen hogy milyenre
  ...
  @valkes:
    mov   ecx,[drb485]                  // Ennyien "vannak"
    dec   ecx                           // A legnagyobb index ez lesz
    push  ecx                           // quicksort 2. paraméter, a jobboldali érték
    push  0                             // quicksort 1. paraméter, a baloldali érték
    call  quicksort                     // Lerendezem, hogy a változás után is jó sorrend legyen
//
// Ha az USB eszköznek is az azonosítója, akkor a változást átvezetem
//
    mov   eax,parrom                    // AX <- ezt kell megváltoztatni, a magas részen hogy milyenre
    mov   edx,offset devusb             // Ez a paraméter címtárolója
...
asm
    ...
    xchg  ebx,[dev485]                  // Ez volt elõtte
    or    ebx,ebx                       // Nulla?
...
asm
  ...
  @folyin:
    ...
    imul  edi,eax,SIZBEL                // A táblázat elejéhez képesti offszet
    add   edi,[belpoi]                  // A táblázatra rátolom
    mov   ax,[edi + LISELE.azonos]      // A küldött azonosító
    mov   ecx,SLLELO                    // A kívánt elõtag
    call  ervazo                        // Azonosító lekérdezés
    jz    @llrend                       // Ez most LED lámpa azonosító
    mov   ax,[edi + LISELE.azonos]      // A küldött azonosító
    mov   ecx,SLNELO                    // A kívánt elõtag
    call  ervazo                        // Azonosító lekérdezés
  ...
  @llrend:
    mov   edx,offset sajpuf             // Az üzenet puffere
    mov   cx,[edi + LISELE.azonos]      // Ezt küldte
    mov   [edx + USB_buf.address],cx    // Ezt kell megváltoztatni
    mov   [edx + USB_buf.tipus],NYILRG  // A kérés kódját kitöltöm
    mov   eax,dword ptr [edi + LISELE.vilrgb]// Az RGB paraméter
    mov   dword ptr [edx + USB_buf.ledszb],eax// Átadom az RGB értékeket
    mov   eax,ERROR_INVALID_DATA        // Hibakód
    mov   cl,[edi + LISELE.nilmeg]      // Az irány
    or    cl,cl                         // Balra?
    jz    @marafa                       // Igen, az is marad
    cmp   cl,1                          // Jobbra?
    jz    @marafa                       // Igen, az is marad
    cmp   cl,2                          // Lámpa?
    jnz   @valveg                       // Nem az, nem jó a kérés kódja
  @marafa:
    mov   [edx + USB_buf.lednir],cl     // Átadom az irány értékeket
  ...
  @nemale:
    ...
    mov   [edx + USB_buf.address],cx    // Ezt kell megváltoztatni
    mov   eax,ERROR_BAD_LENGTH          // Hibakód
    movzx ecx,[edi + LISELE.handrb]     // Ezt küldte hossznak
    cmp   ecx,HANMAX                    // Ennél nagyobb?
  ...
  @marahk:
    ...
    push  edi                           // Elrontom
    push  esi                           // Elrontom
    mov   esi,[edi + LISELE.hantbp]     // Ez a táblázat címe
    lea   edi,[edx + USB_buf.hangtb]    // Ahova tenni kell
    cld                                 // Elõrefele másoljon
    rep   movsb                         // Átmásolom
    pop   esi                           // Vissza a rontott
