...
//
  ...
  HANGEN                  = 'Hangerõ';
//
// A hangerõ konstansai
//
  HANGEP                  = 'Hangerõ';
  HANGMI                  = 'Hangerõ_együtt';
  HANGEA                  = 'Hangerõ_alap';
  HANGEE                  = 'Hangerõ_egyedek';
//
...
//
  TSLF = class(TForm)
    ...
    NyilBalraRadioButton: TRadioButton;
    NyilJobbraRadioButton: TRadioButton;
    ...
    HangeroEgyuttCheckBox: TCheckBox;
    ListaKuldes1: TMenuItem;
    NyilLampaRadioButton: TRadioButton;
    LampaLampaRadioButton: TRadioButton;
    LampaBalraRadioButton: TRadioButton;
    LampaJobbraRadioButton: TRadioButton;
    ...
    procedure HangeroEgyuttCheckBoxClick(Sender: TObject);
    procedure HangListBoxKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure ListaKuldesClick(Sender: TObject);
  private
  ...
  public
    ...
    // Hangváltoztatás engedélyezés
    procedure butten(engede: Boolean);
    // Hangerõ label kipakolás
    procedure halabk(mitakt: Integer);
    // A hangerõ potméter maximumának kiszámítása
    procedure maxkis(melyik: Integer);
  end;
...
var
  ...
  mindrb: Integer;                          // Lista számláló
  hangdb: Integer;                          // A hangleíró elemszáma
  ...
  hangak: array [0..(HANMAX - 1)] of Byte;  // A hangerõk aktuális értéke
...
begin
  ...
  i := GetFileVersionInfoSize(PChar(exenam), Dword(j));
  SetLength(dwbuff, i);
  GetFileVersionInfo(PChar(exenam), j, i, @dwbuff[0]);
	VerQueryValue(@dwbuff[0], DEFSTR, Pointer(ffi), Dword(i));
...
*)
  HangeroEgyuttCheckBox.Checked := sajini.ReadBOOL(HANGEP, HANGMI, False);
  if(HangeroEgyuttCheckBox.Checked) then
  begin
    HangeroTrackBar.Tag := sajini.Readinteger(HANGEP, HANGEA, 1);
    i := 0;
    while(i < HANMAX) do
    begin
      exenam := HANGEE + IntToStr(i);
      hangak[i] := sajini.Readinteger(HANGEP, exenam, 1);
      inc(i);
    end;
  end;
  //
      ...
      case(i) of
        ...
        begin
          ...
          begin
            ...
            tblveg[1].vilrgb.rossze := tblveg[1].vilrgb.rossze XOR 255;
          end
          else
          begin
...
begin
  ...
  iniupd(HANGEP, HANGMI, HangeroEgyuttCheckBox.Checked);
  if(HangeroEgyuttCheckBox.Checked) then
  begin
    iniupd(HANGEP, HANGEA, HangeroTrackBar.Tag);
    i := 0;
    while(i < HANMAX) do
    begin
      s := HANGEE + IntToStr(i);
      iniupd(HANGEP, s, hangak[i]);
      inc(i);
    end;
  end;
...
begin
  ...
  begin
    ...
    //
    irameg := locsta.nyilal;
    LampaBalraRadioButton.Checked := irameg = 0;
    LampaJobbraRadioButton.Checked := irameg = 1;
    LampaLampaRadioButton.Checked := irameg = 2;
    //
    ...
    begin
      case(i) of
        ...
        5, 8, 10, 11, 17:
        begin
          if((irameg = 0) OR (irameg = 2)) then
          begin
            ledlaa[i].Brush.Color := kulcol;
            ledlaa[i].Visible := True;
          end
          else
          begin
            ledlaa[i].Visible := False;
          end;
        end;
        //
...
begin
  ...
  begin
    ...
    //
    irameg := locsta.nyilal;
    NyilBalraRadioButton.Checked := irameg = 0;
    NyilJobbraRadioButton.Checked := irameg = 1;
    NyilLampaRadioButton.Checked := irameg = 2;
    //
      ...
      case(i) of
        ...
        begin
          ...
          begin
            ...
            lednaa[i].Visible := True;
          end
          else
          begin
            lednaa[i].Visible := False;
          end;
        end;
        //
        // Jobbra mutató irányban szereplõ LED szmbólumok be-, vagy kikapcsolása
...
begin
  ...
  begin
    ...
    begin
      if(NOT HangListBox.Enabled) then
      begin
        HangListBox.ItemIndex := HangListBox.Tag;
        butten(True);
      end;
    end
    else
    begin
      HangListBox.Itemindex := locsta.hanakt - 1;
    end;
    //
    // A Timer indítása (új állapot kérés egy idõ után)
    //
...
begin
  ...
  while(i < j) do
  begin
    k := dev485[i].azonos AND $c000;
    if((k = mitind.Tag) OR (mitind.Tag = 0)) then
    begin
      if((k = SLNELO) AND (dev485[i].idever.versih = 1) AND (dev485[i].idever.versil < 1)) then
      begin
        NyilLampaRadioButton.Visible := False;
      end
      else
      begin
        NyilLampaRadioButton.Visible := True;
      end;
      if((k = SLLELO) AND (dev485[i].idever.versih = 1) OR (dev485[i].idever.versil < 1)) then
      begin
        LampaLampaRadioButton.Checked := True;
        LampaLampaRadioButton.Visible := False;
        LampaBalraRadioButton.Visible := False;
        LampaJobbraRadioButton.Visible := False;
      end
      else
      begin
        LampaLampaRadioButton.Visible := True;
        LampaBalraRadioButton.Visible := True;
        LampaJobbraRadioButton.Visible := True;
      end;
      k := Length(listoi);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        //
        // Nem sikerült az indítás, a hibáról üzenek
        //
...
begin
  ...
  begin
    ...
    if(LampaLampaRadioButton.Checked) then
    begin
    ...
    begin
      nilmeg := 0;
      if(LampaJobbraRadioButton.Checked) then
      begin
        nilmeg := 1;
      end;
      i := SLLDLL_LEDNyil(rgbert, nilmeg, aktadr);
    end;
    if(i <> NO_ERROR) then
    begin
...
begin
  ...
  begin
    ...
    if(NyilLampaRadioButton.Checked) then
    begin
      i := SLLDLL_LEDLampa(rgbert, aktadr);
    end
    else
    begin
      nilmeg := 0;
      if(NyilJobbraRadioButton.Checked) then
      begin
        nilmeg := 1;
      end;
      i := SLLDLL_LEDNyil(rgbert, nilmeg, aktadr);
...
procedure TSLF.maxkis(melyik: Integer);
var
  i: Integer;
  mitara, mithan: Extended;
begin
  i := 0;
  if(melyik > -1) then
  begin
    ...
    begin
      if(hangak[i] <> 0) then
      begin
        mithan := 63.0 / hangak[i];
        if(mitara > mithan) then
        begin
          mitara := mithan;
        end;
      end;
    ...
    HangeroTrackBar.Max := Round((hangak[melyik] * mitara) - 0.0000001);
    halabk(melyik);
  end;
end;

procedure TSLF.halabk(mitakt: Integer);
var
  s: String;
begin
  s := Format(HANGES,[HangeroTrackBar.Min, HangeroTrackBar.Max, mitakt]);
...
begin
  if(lehlis) then
  begin
    i := HangeroTrackBar.Position;
    halabk(i);
    if(HangeroEgyuttCheckBox.Checked) then
    begin
      harany := i / HangeroTrackBar.Tag;
      i := 0;
      while(i < hangdb) do
      begin
        if(i <> aktind) then
        begin
          if(hangtb[i].hangso = 49) then
          begin
            hangtb[i].hanger := 0;
          end
          else
          begin
            j := Round((hangak[i] * harany) + 0.000000000001);
            if(j < 1) then
            begin
              j := 1;
            end;
    ...
    begin
      HangeroEgyuttCheckBox.Enabled := i > 0;
    end;
    frisha;
  end;
end;
...
var
  i, j: Integer;
  procedure kitesz(melyik: Integer);
  begin
    if(hangtb[melyik].hangso < 49) then
    begin
      HangListBox.Items[melyik] := Format(HANGST, [HangmagassagComboBox.Items[hangtb[melyik].hangso], hangtb[melyik].hangho, hangtb[melyik].hanger]);
    end
    else
    begin
      HangListBox.Items[melyik] := Format(SZUNET, [HangmagassagComboBox.Items[hangtb[melyik].hangso], hangtb[melyik].hangho]);
    end;
  end;
begin
...
var
  ...
  s: String;
begin
  i := hangtb[hovate].hangso;
  HangmagassagComboBox.Itemindex := i;
  HanghosszComboBox.Text := IntToStr(hangtb[hovate].hangho);
  if(i = (HangmagassagComboBox.Items.Count - 1)) then
  begin
    HangeroTrackBar.Position := 0
  end
  else
  begin
...
begin
  if(HangListBox.Enabled) then
  begin
    aktind := HangListBox.Itemindex;
    if(HangeroEgyuttCheckBox.Checked) then
    begin
      HangeroTrackBar.Tag := hangak[aktind];
    end;
    maxkis(aktind);
    lehlis := False;
    reckit(aktind);
...
procedure TSLF.butten(engede: Boolean);
begin
  LejatszasButton.Enabled := engede;
  HangeroTrackBar.Enabled := engede;
  HangeroTrackBar.SliderVisible := engede;
  HanghosszComboBox.Enabled := engede;
  HangmagassagComboBox.Enabled := engede;
  HangListBox.Enabled := engede;
  if(engede) then
  begin
    HanghosszComboBox.Color := clWindow;
    HangmagassagComboBox.Color := clWindow;
    HangeroEgyuttCheckBox.Enabled := HangeroEgyuttCheckBox.Checked OR (HangeroTrackBar.Position > 0);
  end
  else
  begin
    HanghosszComboBox.Color := clBtnface;
    HangmagassagComboBox.Color := clBtnface;
    HangeroEgyuttCheckBox.Enabled := False;
  end;
end;
//
...
begin
  ...
  begin
    HangListBox.Tag := HangListBox.Itemindex;
    butten(False);
  end;
end;
//
...
procedure TSLF.ListaKuldesClick(Sender: TObject);
var
  i: Integer;
begin
  if(drb485 > 0) then
  begin
    ...
    while(i < drb485) do
    begin
      tblveg[i].azonos := dev485[i].azonos;
      case(dev485[i].azonos AND $c000) of
        SLLELO, SLNELO:
        begin
          case(mindrb AND 7) of
            0:
            begin
              tblveg[i].vilrgb.rossze := 0;
              tblveg[i].vilrgb.gossze := 0;
              tblveg[i].vilrgb.bossze := 0;
            end;
            1:
            begin
              tblveg[i].vilrgb.rossze := 255;
              tblveg[i].vilrgb.gossze := 0;
              tblveg[i].vilrgb.bossze := 0;
            end;
            2:
          ...
          tblveg[i].nilmeg := (mindrb AND $18) SHR 3;
        end;
        SLHELO:
        begin
          tblveg[i].handrb := 0;
    ...
    SLDLL_SetLista(drb485, tblveg);
  end;
end;

procedure TSLF.HangeroEgyuttCheckBoxClick(Sender: TObject);
var
  i, j: Integer;
begin
  if(HangeroEgyuttCheckBox.Checked) then
  begin
    HangeroTrackBar.Tag := HangeroTrackBar.Position;
    i := 0;
    j := hangdb;
    while(i < j) do
    begin
    ...
    maxkis(HangListBox.Itemindex);
  end
  else
  begin
    HangeroTrackBar.Max := 63;
    halabk(HangeroTrackBar.Position);
  end;
end;

procedure TSLF.HangListBoxKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
