...
\begin{document}
	...
	Az elosztott rendszerek lényege, hogy van egy alkalmazás, amivel különböző kliensprogramok kommunikálhatnak, ez a szerveralkalmazás, amely -- általában -- egy másik számítógépen fut, amely számítógép ráadásul egy másik hálózat tagja, tehát a két gép közötti adatátvitel az internet segítségével valósítható meg.
	Kliens-szerver architektúrát különböző módokon építhetünk fel, amelyek a témám szempontjából csak annyiból fontosak, hogy például szolgáljanak programozási nyelvek közötti kapcsolatteremtés alkalmazási lehetőségeire.
	\section{Gondolataim a streamalapú kommunikációról}
	Az elosztott alkalmazások ezen változatainál igazából a két forráskód nyelve megegyezik, és a téma szempontjából egy tanulsággal szolgál: kell egy közös nyelv, egy szerződés a két fél között, amelytől egyik fél sem térhet el: a streamalapú kommunikáció ezt az alábbi módon valósítja meg: szövegesen eljuttatja a szerver számára a végrehajtani kívánt metódus nevét és paramétereit. 
	Például egy kliensalkalmazásban elküldhetjük az alábbi üzenetet a streamen keresztül: \textit{"ADD|2|3"}, ezt a szerver a várakozás állapotában megkapja, rá is illeszti az üzenet első tagját ("ADD") az általa ismert parancsokra, és amennyiben ismeri az "ADD" parancsot, meghívja rá a saját Add(int a, int b)-függvényét, majd egy hasonló stream üzenetet küld vissza a kliens számára \textit{"OK|5"}. Látjuk, hogy van hasonlóság a küldött üzenetek között, van a két fél között egy megállapodás, egy szabvány, ami leírja a kommunikációt. Ez már önmagában nevezhető egy közös nyelvnek a kommunikációban résztvevő két komponens között. A szerver ismeri az $\mathbb{A} = \{"ADD", "SUB", "MUL", "DIV", "EXIT"\}$ parancsokat, tehát $\mathbb{A}$-halmaz elemeit, ezen halmaz elemeit bátran nevezhetjük a közös nyelvet leíró kulcsszavaknak, amelyekkel a távoli számítógépen metódusokat hívhatunk meg), és a kliensnek erről feltétlenül tudnia kell, hogy ismerje ezen parancsok meghívási módját (kvázi az üzenetküldésre használt nyelv szintaxisát!), valamint ezeket meghívva milyen eredményt várhat.
	Ha továbbgondoljuk, a nyelv szemantikája is megadható: tegyük fel, hogy az előbb felsorolt parancsok használatához szükség van bejelentkezésre. Bejelentkezik \textit{"LOGIN|username|password"} üzenettel a szerverre, amely eldönti, hogy a bejelentkezési adatok passzolnak-e az általa eltárolt felhasználói rekordok bármelyikével, amennyiben igen, bejelentkezteti a felhasználót a szerverre. Ekkor már a többi parancs is megnyílik a számára, a \textit{"LOGIN"}-parancs letiltásra kerül. Ha a bejelentkezett felhasználó \textit{"LOGIN|username"}-parancsot ismételten kiadná, kapna a szervertől egy \textit{"ERROR|ALREADY\_LOGGED\_IN"} üzenetet. Szintaktikailag ugyan helyes parancsot adott ki a felhasználó -- azaz az általa futtatott kliensprogram --, de már a parancs meghívása ebben a kontextusban (itt a kontextus alatt kvázi a session\footnote{TODO: session...}, a kliens-szerver kapcsolat fennállásának idejét vehetjük) már egyszer megtörtént, így a megkapott hibaüzenetben tulajdonképpen egy \textit{szemantikai hibáról} kap tájékoztatást a felhasználó.
	...
	Az alábbi példában vizsgáltam a referencia szerinti paraméterátadás működőképességét is, ez abban nyilvánult meg, hogy mind Delphi, mind C\# oldalon meghirdettem egy kimenő (out) paramétert, ami azt jelzi a fordító számára, hogy ez a változó majd az eljárás folyamán fog beállításra kerülni, és értékét -- mivel memóriacímet adtunk át -- a függvényt hívó környezetből is el tudjuk érni.
	Ez akkor hasznos különösen, amikor egynél több dologgal kéne visszatérnünk Például a függvény a lefutás sikerességet jelző kóddal tér vissza, de például beállít egy string változót, amely értékét szintén szeretnénk a hívás kontextusában látni.
	...
	\begin{figure}[h!]
		...
		\label{string_delphi}
	\end{figure}
	 A marshaller alapértelmezés szerint \verb*|PWideChar|-t várna, mivel a C\# oldalon nem alkalmaztunk \verb*|[MarshalAs(UnmanagedType.BStr)]| attribútumot a külső eljárás deklarációjában.
	 Megoldás lehet, hogy Delphiben minden átadni, visszaadni kívánt stringet \verb*|PWideChar|-típusúra módosítsuk, adjuk át referencia szerint, így a C\# oldalán az alapértelmezésnek megfelelünk.
	\begin{figure}[h!]
		\centering
	...
	Ez az eljárás a gRPC megoldására hasonlít \ref{grpc} (egy közös proto nyelv), a C\# és a Delphi között az XML tud közös nyelvként (hídként) funkcionálni.
