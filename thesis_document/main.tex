...
\begin{document}
	...
	A Marshalling primitív típusok esetén abszolút működőképes, összetett adatszerkezetekre viszont inkább a szabványos szöveges formátumokat érdemes alkalmazni tapasztalatom szerint. Ha ezt elfogadjuk, akkor érdemes megismerkedni a szerializáció és deszerializáció folyamatával. %TODO: befejezni
	\section{Szerializáció}
		\cite{sof_serialization}
	\section{Marshalling és szerializáció}
		\cite{marshalling_serialization}
	%todo: ezeket szétosztani
	...
	Például egy kliensalkalmazásban elküldhetjük az alábbi üzenetet a streamen keresztül: \textit{"ADD|2|3"}, ezt a szerver a várakozás állapotában megkapja, rá is illeszti az üzenet első tagját ($"ADD"$) az általa ismert parancsokra, és amennyiben ismeri az $"ADD"$ parancsot, meghívja rá saját $Add(int\ a,\ int\ b)$-függvényét, majd egy hasonló stream üzenetet küld vissza a kliens számára: \textit{"OK|5"}. \cite{elearning_stream}
	
	Láthatjuk, hogy hasonlóság a küldött üzenetek között, van a két fél között egy megállapodás, egy szabvány, ami leírja a kommunikáció formáját. Elméletem szerint ez már önmagában nevezhető egy közös nyelvnek a kommunikációban résztvevő két komponens között. 
	
	A szerver ismeri az $\mathbb{A} = \{"ADD", "SUB", "MUL", "DIV", "EXIT"\}$ parancsokat, tehát $\mathbb{A}$-halmaz elemeit, ezen halmaz elemeit bátran nevezhetjük a közös nyelvet leíró kulcsszavaknak, amelyekkel a távoli számítógépen metódusokat hívhatunk meg), és a kliensnek erről feltétlenül tudnia kell, hogy ismerje ezen parancsok meghívási módját (kvázi az üzenetküldésre használt nyelv szintaxisát!), valamint ezeket meghívva milyen eredményt várhat.
	...
	%todo: szintaktika - szemantika footnoteok
	Szintaktikailag\footnote{Szintaktika} ugyan helyes parancsot adott ki a felhasználó -- azaz az általa futtatott kliensprogram --, de már a parancs meghívása ebben a kontextusban (ebben a párhuzamban kontextusnak számíthatjuk a sessiont)\footnote{Egy többfelhasználós rendszerben munkamenetnek (session) nevezzük azt az időszakot, ami eltelik a felhasználó be- és kijelentkeztetése között, ekkor az szabadon végezheti a dolgát, nem kell minden egyes funkció használatához bejelentkeznie.} már egyszer megtörtént, így a megkapott hibaüzenetben tulajdonképpen egy \textit{szemantikai hibáról}\footnote{Szemantika} kap tájékoztatást a felhasználó.
	
	\section{RESTful alkalmazások}
	
	\section{Google RPC és a Proto-nyelv}
	\label{grpc}
	A Google RPC, röviden gRPC a Google jóvoltából készült nyílt forráskódú RPC-keretrendszer. Remote Procedure Call-nak\footnote{magyarul: Távoli eljáráshívás} nevezzük, amikor $A$ processz meghívja $B$ processz valamely meghirdetett szolgáltatását, így $B$ a kért műveletet implementációja szerint elvégzi, majd $A$ részére válaszüzenetben visszajuttatja az általa kért művelet eredményét. Így $A$-nak nem kell törődnie a végrehajtott algoritmus részleteivel, ennek implementációja és futtatása $B$ processzre delegálódik. Ez lehetőséget teremt arra, hogy bizonyos kódrészleteket egy másik számítógépre, egy szerverre helyezzünk át, ettől lesz a hívás 'távoli'.
	...
	%TODO: RPC - távirányítós példát kifejteni
	...
	Ami weboldalaknál a HTML és a JSON-formátum, az a gRPC esetén a Proto-nyelv, ami átjárást biztosít akár más nyelven írt programok részére is. Amely nyelv képes implementálni a gRPC-t, és a Proto-nyelvből le tudja gyártani maga forráskódját, és a forráskódjából generálható a Proto-formátum, az alkalmas ezen a nyelven keresztül összekapcsolódni más, ugyanezt a keretrendszert alkalmazó szerverprogrammal is. A Proto-nyelv választott témám szempontjából kruciális, mivel kifejezetten erre a célra lett kitalálva, hogy programozási nyelvek felett álljon, ezért úgy gondolom, érdemes tovább vizsgálnunk a nyelvben rejlő lehetőségeket.

	Miért kell ehhez egy külön nyelv? Nem lehetne maradni a jól bevált JSON-nél? A Proto nyelv további előnyökkel jár, amelyeket sem a JSON, sem az XML, sem bármilyen  más szerializált formátum nem tud biztosítani: az adatszerkezeteket szigorúan típusosan írja le. Egy úgynevezett proto fájlban történik az 

	Ezenkívül nemcsak adatok, hanem szolgáltatások leírására egyaránt alkalmas a Proto-nyelv, ezért is érdemes az RPC ezen változatát használni, mivel nagyobb rugalmasságot biztosít, mint egy átlagos szerializált formátum. Lényegében elég ezt a proto fájlt elkészítenünk, ennek környezetét, a kliens- és szerveroldali stubok forráskódjának vázát az adott nyelv szabályrendszere szerint képes lesz legyártani ebből. Ez a legtöbb RPC-keretrendszernél nem áll fenn.

	Mint azt említettem, Proto-nyelvben a változókat csak típussal együtt vehetünk fel\footnote{Ezen tulajdonság miatt a Proto szigorúan típusos nyelvnek számít.}, osztályokat \verb*|message| kulcsszóval látja el, a mezőket számokkal látjuk el. 
	%todo: javítani, kifejteni

	RPC-vel hívható metódusokat definiálhatunk paraméterrel és visszatérési típussal. Ezek mind szerver, mind kliens oldalon meg fognak jelenni gRPC által generált szabványos formában. 
	%todo: kép egy .proto fájlról - beadandómból!
	További előnye, hogy az adatokat a lehető legtömörebb bináris formátumban küldi át a hálózaton, a JSON-formátum ezzel szemben egy az egyben szövegesen kerül átadásra.
	\cite{grpc_rest}
	\begin{figure}[h!]
		\centering
		\includegraphics[scale=0.35]{grpc_json}
		\caption{JSON és Protocol Buffers teljesítményének összehasonlítása}
		\label{fig:grpc_json}
	\end{figure}
	%TODO: magyarázat az ábrához
	...
	Egy mentális egészségfejlesztésre használatos alkalmazást fejlesztésére vállalkoztunk 2022 szeptemberében, amely elméleti alapjait Somodi László futballedző munkásságának köszönhetjük, ezek gondolati vonulatairól titoktartási szerződésünk
	%TODO: Ezt akár betehetnénk ide vagy hivatkozhatnánk rá valamilyen formában
	...
	\subsubsection{A probléma leírása:} Az \verb*|SLDLL_Open|-függvény várja az ablakos alkalmazás handle-mezőjét, amely az üzenetküldést engedélyezi az SLDLL-függvények részéről.
	...
	\section{A relayDLL-ben eldobott kivételek nem kezelhetőek}
	\subsection{Megoldás} Hibakódokkal jelezzük a C\# felé, ha esetleg hibára futott valamelyik függvény, ebből a hibakódból a C\# tudni fogja, hogy milyen típusú kivételt dobjon az asztali alkalmazás számára, amelyek már lehetnek egyénileg elkészített Exception-objektumok.
	...
	\begin{enumerate}
		...
		\item Ezen az üzeneten keresztül az operációs rendszer meghívja a Form \verb*|WndProc| metódusát\footnote{ezt azért teheti meg, mivel az \textit{SLDLL\_Open} hívásakor átadott Form Handle-n keresztül ismeri, hogy merre továbbítsa a rendszerüzeneteket.}.
		\item Amennyiben a felmérés sikeresen lezajlott, a \verb*|WndProc| meghívja a dev485 és drb485 változókat beállító \verb*|SLDLL_ListElem|-függvényt, mielőtt az \verb*|SLDLL_Felmeres|-re kerülne a sor. A megfelelő hívási szekvencia így automatikusan biztosított.
	\end{enumerate}
	\section{Eszközök azonosítóinak, típusainak átadása}
	\subsection{Megoldás}
	A Delphiben tárolt tömb (dev485) eszközeinek legfontosabb mezőjét (ami az eszközt azonosítója, tehát az azonosítót XML-fájlba kimentjük vagy JSON-formátumú szöveggé alakítjuk. Az azonosító önmagában meghatározza az eszköz típusát, úgyhogy elég azt átadni. \cite{sof_xmlcreate}
