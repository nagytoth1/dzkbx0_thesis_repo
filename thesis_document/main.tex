...
\begin{document}
	...
	\begin{itemize}
		\item FELMOK (1)  LParam -> a felmérés által talált eszközök száma                  
		\item FIRMUZ (3)  LParam -> a frissítés adatait tartalmazó struktúra (UPGPCK) címe                          
		\item LEDRGB (5)  LParam -> az általános állaptinformáció struktúra (ELVSTA) címe    
		\item NYIRGB (6)  LParam -> az általános állaptinformáció struktúra (ELVSTA) címe    
		\item HANGEL (7)  LParam -> az általános állaptinformáció struktúra (ELVSTA) címe    
		\item STATKV (8)  LParam -> az általános állaptinformáció struktúra (ELVSTA) címe    
		\item LISVAL (9)  LParam -> a táblázat végrehajtás végének hibakódja                 
		\item FELMHK (-3) LParam -> a felmérés hibás                     
	\end{itemize}
	
	Az egyes funkciók visszatérési kódja tájékoztat a hívás sikeres vagy            
	sikertelen voltáról. Ha a visszatérési kód NO\_ERROR (0), akkor a hívás         
	sikeres volt. Ha nem NO\_ERROR (0) az érték a kód tájékoztat a sikertelenség    
	...
	A hívási sorrend (szekvencia) következő lépése az \verb*|SLDLL_Felmeres|-függvény meghívása, amely az USB-porton észlelt eszköz(ök) felmérését indítja el. A nyomtatókábel USB-B vége a számítógép bármely -- szintén -- USB-B portjára csatlakoztatható. Kifejezetten előnyös tulajdonsága a metódusnak, hogy képes eldönteni, pontosan melyik porton történt meg a csatlakoztatás, így nem kell egy-egy kijelölt portot megjelölni, amelyen kizárólag ezen eszközök detektálásával foglalkozik a program. Érdekes kérdés, hogy mi történne, ha több porton csatlakoztatnánk a nyomtatókábelt további eszközök bekapcsolásával. %todo: megvizsgálni
	...
	\begin{itemize}
		\item \verb*|NO_ERROR|: Nem történt hiba
		\item \verb*|ERROR_DLL_INIT_FAILED|: Az SLDLL\_Open még nem lett meghívva
	...
	\begin{enumerate}
		\item Az eszköz lámpa vagy nyíl típusú:
			\begin{itemize}
			...
			\end{itemize}
		\item Az eszköz hangszóró típusú:
			\begin{itemize}
	...
	\end{enumerate}
	Az első esetben szimplán nem érzékelte az USB-porton lévő eszközöket (1626-os hiba), mígy az utóbbi esetben az alábbi hibakódot dobja az eszközök felméréskor.
	\subsubsection{Megoldás:} Az általunk használt telefonkábel rosszul volt bekötve, a kábel két végén a középső erek bekötése egymással ellentétes volt.
